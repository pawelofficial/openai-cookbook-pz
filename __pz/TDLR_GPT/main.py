...
#collection = chroma_client.create_collection(name="bullish_bearish")
collection = chroma_client.get_or_create_collection(name="bullish_bearish")  # get or create 
...
if 0: # loop for loading data into chroma
    ...
    for df_chunk  in gen_df:
        ...
        _,text_summary=oa_enrich_text(txt,prompt='if text is bullish about bitcoin, type bullish, if bearish then type bearis, if neither type neither')
        bullish.append(text_summary)
        #print('\n',text_summary)
        #print('------')
...
question='Will bitcoin go up or go down?'
